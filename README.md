...
- **五种权限模式**：
  ...
  - `auto` 自动：文件与命令直接执行，但**每条命令先由工作模型审查**，危险操作（删除用户文件、清库、`git push`、sudo 等）转用户审批
  - `full` 完全访问：不审批，但命令与用途仍会通知用户
  - `yolo` 静默：权限最高且**不推送任何中间通知**，用户只收到最终结果
- **个人工作区**：每个 QQ 号独立工作区，默认 `data/agent/workspace/<qq号>`
- **聊天内审批**：非 full 权限档下 bash 命令需 `.agent approve` 批准（不带 id 时处理该用户最近一次请求）；每条命令都必须带 `purpose`，审批与执行告知都会带上用途，full 权限下也会推送执行通知
